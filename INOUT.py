...
from shapely.geometry import Point as DOT
...
class INOUT:
    def __init__(self):
        self.DETECTOR = SCRFD("/home/foziljon/PROJECTS/TRACK/models/det_500m.onnx")
        self.DETECTOR.prepare(-1)
        self.RECOGNITION = ArcFaceONNX("/home/foziljon/PROJECTS/TRACK/models/w600k_mbf.onnx")
        self.RECOGNITION.prepare(-1)
        
        self.MODEL = YOLO("/home/foziljon/PROJECTS/TRACK/models/yolov8n.pt")
        self.MODEL.fuse()
        
        self.DATABASE = self.database_loader('/home/foziljon/PROJECTS/TRACK/dataset')
    ...
    def OUT(self, source):
        ...
        while cap.isOpened():
            ...
            for bbox, tracker_id in zip(BBOX,TRACKER_ID):
                ...
                if not self.POLYGON.contains(DOT(x,y)):
                    continue
            ...
            self.OUT_LINE_COUNTER.update(BBOX, TRACKER_ID, face_id=FACEID, out = True)
            self.LINE_ANNOTATOR.annotate(frame=frame, line_counter=self.OUT_LINE_COUNTER, text = "OUT")

            frame = self.BOX_ANNOTATOR.annotate(frame=frame, bboxes=BBOX, colors = COLOR, labels=labels)
            cv2.polylines(frame, [self.OUT_RECTANGLE], True, (0,255,255), thickness=5)
            end = time.time()
            cv2.putText(frame, f"FPS: {1./(end-start)}", (60,60), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), thickness=2)
